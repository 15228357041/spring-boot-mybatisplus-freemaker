...
import static cn.chinau8.util.StringUtil.lineToHump;
...
public class MybatisGeneratorUtil {
	...
	// 列表jsp 模板路径
	private static String jsp_vm = "/template/jsp.vm";
	...
	public static void generator(
		...
		if (os.toLowerCase().startsWith("win")) {
			URL a = MybatisGeneratorUtil.class.getResource("");
		...
		String controllerPath = basePath + module + "/" + module + "-server" + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/server/controller";
		String jspPath = basePath + module + "/" + module + "-server" + "/src/main/webapp/WEB-INF/jsp";
		for (int i = 0; i < tables.size(); i++) {
			...
			String controllerDir = controllerPath + "/" + StringUtil.toLowerCaseFirstOne(model);
			String controller = controllerPath + "/" + StringUtil.toLowerCaseFirstOne(model) + "/" + model + "Controller.java";
			String jspDir = jspPath + "/" + mapper;
			String jsp = jspPath+ "/"+ mapper+ "/" +tableName+"_list.jsp";
			// 生成service
			...
			if(createController) {
				...
				File controllerDirFile = new File(controllerDir);
				if (!controllerFile.exists()) {
					controllerDirFile .mkdir();
					VelocityContext context = new VelocityContext();
					context.put("package_name", packageName);
					context.put("model", model);
				...
				if (!jspFile.exists()) {
					...
					context.put("mapper", StringUtil.toLowerCaseFirstOne(model));
					context.put("ctime", ctime);
					VelocityUtil.generate(jsp_vm, jsp, context);
